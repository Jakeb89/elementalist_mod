...
public abstract class ElementOrb extends AbstractOrb {
	...
	public void checkShatter() {
		/*
		 * if (amount <= 0) { onShatter(); return; }
		 */
	}

	public void onShatter() {
		/*
		 * AbstractPlayer p = AbstractDungeon.player; p.orbs.remove(this);
		 * 
		 * int index = p.orbs.indexOf(this);
		 * 
		 * AbstractOrb orbSlot = new EmptyOrbSlot(); for (int i = index + 1; i <
		 * p.orbs.size(); i++) { Collections.swap(p.orbs, i, i - 1); }
		 * p.orbs.set(p.orbs.size() - 1, orbSlot); for (int i = index; i <
		 * p.orbs.size(); i++) { ((AbstractOrb) p.orbs.get(i)).setSlot(i, p.maxOrbs); }
		 * 
	...
	public void render(SpriteBatch sb) {
		...
		if (element == Element.FIRE) {
			sb.draw(imgExtra, this.cX - 48.0F + this.bobEffect.y / 4.0F, this.cY - 48.0F + this.bobEffect.y / 4.0F, 48.0F, 48.0F, 96.0F, 96.0F, this.scale, this.scale, rotation, 0,
				0, 96, 96, false, false);
		}
		if (element == Element.AIR) {
			sb.draw(imgExtra, this.cX - 48.0F + this.bobEffect.y / 4.0F, this.cY - 48.0F + this.bobEffect.y / 4.0F, 48.0F, 48.0F, 96.0F, 96.0F, this.scale, this.scale, -rotation,
				0, 0, 96, 96, false, false);
		}
		if (element == Element.EARTH) {
			sb.draw(imgExtra, this.cX - 48.0F + this.bobEffect.y / 4.0F, this.cY - 48.0F + this.bobEffect.y / 4.0F, 48.0F, 48.0F, 96.0F, 96.0F, this.scale, this.scale,
				(float) (Math.sin(3.14f * rotation / 90f) * 5f), 0, 0, 96, 96, false, false);
		}
		sb.draw(img, this.cX - 48.0F + this.bobEffect.y / 4.0F, this.cY - 48.0F + this.bobEffect.y / 4.0F, 48.0F, 48.0F, 96.0F, 96.0F, this.scale, this.scale, rotation, 0, 0, 96,
			96, false, false);
		sb.draw(img2, this.cX - 48.0F + this.bobEffect.y / 4.0F, this.cY - 48.0F + this.bobEffect.y / 4.0F, 48.0F, 48.0F, 96.0F, 96.0F, this.scale, this.scale, 0f, 0, 0, 96, 96,
			false, false);
		sb.draw(img3, this.cX - 48.0F + this.bobEffect.y / 4.0F, this.cY - 48.0F + this.bobEffect.y / 4.0F, 48.0F, 48.0F, 96.0F, 96.0F, this.scale, this.scale, -rotation, 0, 0, 96,
			96, false, false);
		sb.draw(img4, this.cX - 48.0F + this.bobEffect.y / 4.0F, this.cY - 48.0F + this.bobEffect.y / 4.0F, 48.0F, 48.0F, 96.0F, 96.0F, this.scale, this.scale, 0f, 0, 0, 96, 96,
			false, false);
		sb.draw(img5, this.cX - 48.0F + this.bobEffect.y / 4.0F, this.cY - 48.0F + this.bobEffect.y / 4.0F, 48.0F, 48.0F, 96.0F, 96.0F, this.scale, this.scale, 0f, 0, 0, 96, 96,
			false, false);
		sb.draw(imgBevel, this.cX - 48.0F + this.bobEffect.y / 4.0F, this.cY - 48.0F + this.bobEffect.y / 4.0F, 48.0F, 48.0F, 96.0F, 96.0F, this.scale, this.scale, 0f, 0, 0, 96,
			96, false, false);

		if (element == Element.WATER) {
			sb.draw(imgExtra, this.cX - 48.0F + this.bobEffect.y / 4.0F, this.cY - 48.0F + this.bobEffect.y / 4.0F, 48.0F, 48.0F, 96.0F, 96.0F, this.scale, this.scale,
				(float) (Math.cos(3.14f * rotation / 90f) * 5f), 0, 0, 96, 96, false, false);
		}
		...
		try {
			...
			for (int i = 0; i < Math.min(12, this.amount); i++) {
				...
				dx = position.x;
				dy = position.y;
				sb.draw(img4, this.cX - 48.0F + this.bobEffect.y / 4.0F + dx * 32f, this.cY - 48.0F + this.bobEffect.y / 4.0F + dy * 32f, 48.0F, 48.0F, 96.0F, 96.0F,
					this.scale * 0.5f, this.scale * 0.5f, 0f, 0, 0, 96, 96, false, false);
			}
	...
	private void renderHighlight(SpriteBatch sb, boolean active) {
		...
		if (active) {
			sb.draw(orbHighlightTexture, this.cX - 64.0F + this.bobEffect.y / 4.0F, this.cY - 64.0F + this.bobEffect.y / 4.0F, 64.0F, 64.0F, 128.0F, 128.0F, drawScale, drawScale,
			...
			for (int i = 0; i < MagusStaff.charges; i++) {
				...
				sb.setColor(col);
				sb.draw(orbHighlightPassiveTexture, this.cX - 64.0F + this.bobEffect.y / 4.0F, this.cY - 64.0F + this.bobEffect.y / 4.0F, 64.0F, 64.0F, 128.0F, 128.0F, drawScale,
					drawScale, this.angle * MagusStaff.charges + meteorAngle, 0, 0, 128, 128, false, false);
			}
		}
	}

	private boolean shouldPassiveHighlight() {
		if (!AbstractDungeon.player.hasRelic(MagusStaff.ID))
			return false;
	...
	protected void renderText(SpriteBatch sb) {
		...
		switch (element) {
		case FIRE:
			elementColor = Color.RED.cpy();
			break;
		case WATER:
			elementColor = Color.BLUE.cpy();
			break;
		case EARTH:
			elementColor = Color.YELLOW.cpy();
			break;
		case AIR:
			elementColor = Color.GREEN.cpy();
			break;
		}
	...
	private void renderParticles(SpriteBatch sb) {
		...
		/*
		 * for(AbstractPower power : AbstractDungeon.player.powers) { if(power
		 * instanceof ConfluencePower) { drawSynergy = true; }else if(power instanceof
		 * AstralFormPower) { drawSynergy = true; } }
		 * 
		 * for(AbstractCard card : AbstractDungeon.player.hand.group) { if(card
		 * instanceof Concentricity) { drawSynergy = true; } }
		 */
		drawSynergy = true;
